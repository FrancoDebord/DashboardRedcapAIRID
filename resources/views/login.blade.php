...
<body class="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 flex items-center justify-center">
  <div class="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
    <!-- Visuel / Illustration -->
    <div style="background: #c10202" class="hidden md:flex items-center justify-center bg-gradient-to-b from-indigo-600 to-indigo-500 rounded-2xl shadow-lg p-8 text-white">
      <div class="space-y-6 max-w-md">
        <h1 class="text-3xl font-extrabold">Bienvenue chez <span class="text-yellow-300">DashBoard ATSB Project</span></h1>
        <p class="opacity-90">Connectez-vous pour accéder à votre tableau de bord et gérer vos projets. Si vous n'avez pas de compte, créez-en un en quelques secondes.</p>
        <ul class="space-y-2 text-sm opacity-90">
          <li>✅ Tableau de bord personnalisé</li>
          <li>✅ Statistiques en temps réel</li>
          <li>✅ Support & sauvegarde</li>
        </ul>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-md p-8 flex flex-col justify-center">
      <div class="mb-6 text-center">
        <a href="/" class="inline-block">
          <img src="{{ asset("storage/assets/logo/airid.png") }}" alt="Logo" class="mx-auto " style="width: 234px">
        </a>
      ...
      <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div x-data="{ show: false }">
          ...
          <div class="mt-1 relative">
            ...
            <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 px-3 flex items-center text-gray-500">
              <template x-if="!show">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 3C6 3 2.7 5.1 1 8.5 2.7 11.9 6 14 10 14s7.3-2.1 9-5.5C17.3 5.1 14 3 10 3zM10 12a2 2 0 110-4 2 2 0 010 4z"/></svg>
              </template>
              <template x-if="show">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M4.03 3.97a.75.75 0 10-1.06 1.06l1.1 1.1A9.9 9.9 0 001 8.5C2.7 11.9 6 14 10 14c1.46 0 2.83-.28 4.06-.77l1.98 1.98a.75.75 0 101.06-1.06L4.03 3.97zM10 12a2 2 0 01-2-2c0-.18.03-.35.08-.51l2.43 2.43c-.16.05-.33.08-.51.08z"/></svg>
              </template>
            </button>
          </div>
        ...
        <div class="flex items-center justify-between">
          <label class="inline-flex items-center text-sm">
            <input type="checkbox" name="remember" class="rounded border-gray-200 text-indigo-600 shadow-sm focus:ring-indigo-500">
        ...
        <div>
          <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md font-semibold shadow">
            Se connecter
          </button>
        </div>
      ...
      <footer class="mt-6 text-xs text-gray-400 text-center">
        © {{ date('Y') }} MonApp — Tous droits réservés
      </footer>
    </div>
  </div>
</body>
